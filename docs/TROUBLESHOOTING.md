...
```bash
aws s3 ls
s3_bucket_name = "terraform-state-bucket-goit-devops-project-olegsl"
```
...
aws eks describe-nodegroup \
  ...
  --nodegroup-name goit-devops-project-worker-nodes \
  --region us-west-2
...
aws ecr describe-repository --repository-name microservice-project-ecr --region us-west-2
kubectl get secrets
```
